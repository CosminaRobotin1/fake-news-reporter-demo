...
public class FactCheckService {
    ...
    public FactCheckResponseMessage process(FactCheckRequestMessage message) { // primeste mesajul care vine prin kafka
        try {
            ...
            if (claim.claimReview == null || claim.claimReview.isEmpty()) {//exista claim, dar nu si review
   ...
   /* //pt Python
    private final PythonFactCheckClient pythonClient;

    public FactCheckService(PythonFactCheckClient pythonClient) {
        this.pythonClient = pythonClient;
    }

    public FactCheckResponseMessage process(FactCheckRequestMessage message) {
        try {
            PythonFactCheckRequest request = new PythonFactCheckRequest(
                    message.getRequestId(),
                    message.getReportId(),
                    message.getText()
            );
            //se face HTTP POST;se trimite JSON;se primește răspuns
            PythonFactCheckResponse response = pythonClient.check(request);

            return new FactCheckResponseMessage(
