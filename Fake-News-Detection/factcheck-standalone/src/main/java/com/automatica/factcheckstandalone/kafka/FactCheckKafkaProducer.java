...
// Producer Kafka responsabil cu trimiterea rezultatelor fact-check
// către aplicația FakeNewsReporter
@Service
...
public class FactCheckKafkaProducer {
    ...
    public void send(FactCheckResponseMessage message) {
        kafkaTemplate.send(topic, message.getReportId().toString(), message);//trimitem un mesaj in kafka
    }
